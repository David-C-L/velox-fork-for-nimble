...
std::vector<EncoderEntry<T>> buildDefaultEncoders() {
  ...
  // SIS/realNested+view+ctor and SIS/key_derived+view+ctor are gone. They
  // existed to report the number their +view twins left out: a view built in
  // encodeWith is built outside the timed region, and where a section falls
  // back to MaterializedEncodingView that hides a whole-section decode. Baking
  // a second arm per view arm never covered more than two of them, and the two
  // numbers were never on the same row. Every driver now measures the build
  // through NimbleBenchTargetBase::buildAccessStructure() and writes both
  // build_ns and time_incl_build_ns, so every view arm reports both and none
  // can report only one.
